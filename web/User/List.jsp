            ...
            <table
                ...
                <%
                    ...
                    while (data.next()) {
                        ...
                        String st1 = data.getString("st1");
                        String st2 = data.getString("st2");

                        LocalTime sTime = LocalTime.parse(time, DateTimeFormatter.ofPattern("HH:mm"));
                        sTime = sTime.plusMinutes(Long.parseLong(st1));
                        DateTimeFormatter formatter = DateTimeFormatter.ofPattern("h:mm a");
                        String ftime = sTime.format(formatter);

                        LocalTime rTime = LocalTime.parse(time, DateTimeFormatter.ofPattern("HH:mm"));
                        rTime = rTime.plusMinutes(Long.parseLong(st2));
                        String ttime = rTime.format(formatter);

                        int km = data.getInt("skm2") - data.getInt("skm1");
                        int amount = 0;
                        if (km >= 50) {
                            amount = ((km - 50) * 10) + 200;
                        } else if (km < 50) {
                ...
                <tr>
                    <td> <%=i%> </td>
                    <td> <%=data.getString("route")%> </td>
                    <td> <%=data.getString("bus")%> </td>
                    <td> <%=data.getString("froml")%> </td>
                    <td> <%=ftime%> </td>
                    <td> <%=data.getString("tol")%> </td>
                    <td> <%=ttime%> </td>
                    <td> <%=amount%> </td>
                    <td><a href="Slot.jsp?id=<%=data.getString("sid")%>&amount=<%=amount%>" >Book Now</a></td>
                </tr> 
                <%
                    }
